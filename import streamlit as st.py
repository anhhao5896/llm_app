...
- Libraries available: ggplot2, dplyr, gtsummary, survival, flextable
- For gtsummary tables, save as HTML: as_flex_table(table) %>% save_as_html("table.html")
"""
...
if st.session_state.df is not None:
    ...
    if user_input:
        ...
        - For gtsummary tables, MUST save as HTML using flextable:
        ...
        # Create and save table as HTML using flextable
        table <- tbl_regression(model, exponentiate = TRUE)
        ft <- as_flex_table(table)
        save_as_html(ft, "table.html")
        ...
        ft2 <- as_flex_table(summary_table)
        save_as_html(ft2, "summary.html")
        ```
                        ...
                        for i in range(1, len(code_blocks)):
                            ...
                            with tempfile.TemporaryDirectory() as tmpdir:
                                ...
                                if result['success']:
                                    ...
                                    with st.expander("Show R code that failed"):
...
💡 Powered by R, ggplot2, gtsummary, and flextable | 
🔒 Your data stays private and is not stored |
⚙️ Requires R and packages (ggplot2, dplyr, gtsummary, survival, flextable) installed
</div>
